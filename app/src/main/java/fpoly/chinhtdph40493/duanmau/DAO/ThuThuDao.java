...
public class ThuThuDao {
    ...
    public boolean updateTT(ThuThu thuThu) {
        SQLiteDatabase database = dbHelper.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put("maTT", thuThu.getMaTT());
        values.put("hoTen", thuThu.getHoTen());
        values.put("matKhau", thuThu.getMatKhau());
        long kq = database.update("ThuThu", values, "maTT = ?", new String[]{thuThu.getMaTT()});
        return kq != 0;
    }

    public boolean deleteTT(String id) {
        SQLiteDatabase database = dbHelper.getWritableDatabase();
        long kq = database.delete("ThuThu", "maTT = ?", new String[]{id});
        return kq != 0;
    }
    ...
    public boolean updatePass(String user,String pass, String newPass){
        ...
        if (cursor.getCount() > 0){
            ...
            Log.d("TAG", "Number of rows updated: " + kq);
            if (kq != 0){
                return true;
            }
            return false;
